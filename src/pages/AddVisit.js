...
const Icon = ({ name, size = 16, color = 'currentColor', strokeWidth = 1.8 }) => {
  ...
  const icons = {
    ...
    notes:    <svg style={s} viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>,
...
function AddVisit() {
  ...
  const T = {
    ar: {
      ...
      loadingCities: 'جاري جلب المدن...',
      selectFirst: 'اختر دولة أولاً',
      noCities: 'لا توجد مدن',
      noCity: 'لا توجد مدينة بهذا الاسم',
      citiesAvailable: 'مدينة متاحة',
      locating: 'جاري تحديد الإحداثيات...',
      locationSet: 'تم تحديد الموقع',
      date: 'تاريخ الزيارة',
      rating: 'تقييمك',
      notes: 'ملاحظاتك',
      notesPlaceholder: 'ما الذي أعجبك في هذه الرحلة؟',
      photo: 'صورة من الرحلة (اختياري)',
      changePhoto: 'تغيير الصورة',
      choosePhoto: 'اضغط لاختيار صورة — JPG, PNG بحجم أقصى 5MB',
      save: 'حفظ الزيارة',
      saving: 'جاري الحفظ...',
      successMsg: 'تم حفظ الزيارة!',
      errorMsg: 'حدث خطأ، تأكد من البيانات',
    },
    en: {
      title: 'Add New Visit',
      back: 'Back',
      ...
      selectFirst: 'Select a country first',
      ...
      rating: 'Your Rating',
      notes: 'Your Notes',
      notesPlaceholder: 'What did you enjoy about this trip?',
      photo: 'Trip Photo (optional)',
      changePhoto: 'Change Photo',
      choosePhoto: 'Click to choose a photo — JPG, PNG max 5MB',
      save: 'Save Visit',
      saving: 'Saving...',
      successMsg: 'Visit saved!',
      errorMsg: 'An error occurred, please check your data',
    }
  }[lang];
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060D18', fontFamily: 'Tajawal, sans-serif', direction: lang === 'ar' ? 'rtl' : 'ltr', position: 'relative' }}>
      <style>{`
        ...
        .add-input:focus { border-color: rgba(192,57,43,0.4) !important; }
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: isMobile ? '18px 16px' : '28px 20px', position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <a href="/dashboard" style={{ color: 'rgba(255,255,255,0.5)', textDecoration: 'none', background: 'rgba(255,255,255,0.06)', padding: '7px 14px', borderRadius: '20px', fontSize: '12px', fontWeight: '700', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <Icon name="arrow" size={13} color="rgba(255,255,255,0.5)"/>
          {T.back}
        </a>
        <span style={{ color: 'white', fontSize: isMobile ? '14px' : '16px', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Icon name="plus" size={16} color="#E74C3C" strokeWidth={2.5}/>
          {T.title}
        </span>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={cardStyle}>
            <label style={labelStyle}>
              <Icon name="globe" size={14} color="rgba(255,255,255,0.4)"/>
              {T.selectCountry}
            </label>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: isMobile ? '8px' : '10px', maxHeight: isMobile ? '220px' : '300px', overflowY: 'auto', paddingBottom: '4px' }}>
              {filteredCountries.length === 0 ? (
                ...
                <div key={c.code} className="country-card"
                  onClick={() => {
                    setForm({ ...form, country: c.name, country_code: c.code, latitude: c.lat, longitude: c.lng, city: '' });
                    setCitySearch(''); setShowCities(false); fetchCities(c.nameEn);
                  }}
                  style={{
                    width: cardW, padding: isMobile ? '8px 6px' : '12px 8px',
                    borderRadius: '16px', cursor: 'pointer',
                    background: form.country_code === c.code ? 'linear-gradient(135deg,rgba(192,57,43,0.5),rgba(231,76,60,0.35))' : 'rgba(255,255,255,0.04)',
                    border: `2px solid ${form.country_code === c.code ? '#E74C3C' : 'rgba(255,255,255,0.08)'}`,
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px',
                    transition: 'all 0.2s', flexShrink: 0,
                    boxShadow: form.country_code === c.code ? '0 4px 16px rgba(192,57,43,0.35)' : 'none',
                  }}
                >
          ...
          <div style={{ ...cardStyle, position: 'relative' }}>
            <label style={labelStyle}>
              <Icon name="city" size={14} color="rgba(255,255,255,0.4)"/>
              {T.selectCity}
            </label>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                placeholder={
                  loadingCities ? T.loadingCities :
                  !form.country ? T.selectFirst :
                  cities.length > 0 ? `${cities.length} ${T.citiesAvailable}` : T.noCities
                }
              ...
              {showCities && cities.length > 0 && (
                <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, left: 0, zIndex: 9999, maxHeight: '220px', overflowY: 'auto', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.15)', background: '#162840', boxShadow: '0 16px 40px rgba(0,0,0,0.6)', animation: 'slideDown 0.2s ease' }}>
                  ...
                  {filteredCities.length === 0 ? (
                    ...
                    <div key={i}
                      onMouseDown={() => { setCitySearch(city); setShowCities(false); fetchCityCoords(city, selectedCountry?.nameEn || form.country); }}
          ...
          <div style={cardStyle}>
            ...
            <input className="add-input" type="date" value={form.visit_date} onChange={e => setForm({ ...form, visit_date: e.target.value })} style={inputStyle}/>
          </div>

          {/* ── التقييم ── */}
          <div style={cardStyle}>
            <label style={labelStyle}>
              <Icon name="star" size={14} color="rgba(255,255,255,0.4)"/>
              {T.rating}
            </label>
          ...
          <div style={cardStyle}>
            <label style={labelStyle}>
              <Icon name="notes" size={14} color="rgba(255,255,255,0.4)"/>
              {T.notes}
            </label>
            <textarea className="add-input" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder={T.notesPlaceholder} rows={3} style={{ ...inputStyle, resize: 'none' }}/>
          </div>

          {/* ── الصورة ── */}
          <div style={cardStyle}>
            <label style={labelStyle}>
              <Icon name="camera" size={14} color="rgba(255,255,255,0.4)"/>
              {T.photo}
            </label>
            {photoPreview && (
              <div style={{ marginBottom: '12px', position: 'relative' }}>
                <img src={photoPreview} alt="preview" style={{ width: '100%', height: isMobile ? '160px' : '180px', objectFit: 'cover', borderRadius: '12px', display: 'block' }}/>
                <button type="button" onClick={() => { setPhoto(null); setPhotoPreview(null); }} style={{ position: 'absolute', top: '8px', left: '8px', background: 'rgba(0,0,0,0.7)', color: 'white', border: 'none', borderRadius: '50%', width: '28px', height: '28px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {/* ── زر الحفظ ── */}
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '15px', background: loading ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg,#C0392B,#E74C3C)', color: loading ? 'rgba(255,255,255,0.3)' : 'white', border: 'none', borderRadius: '16px', fontSize: '15px', fontWeight: '800', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Tajawal, sans-serif', boxShadow: loading ? 'none' : '0 8px 24px rgba(192,57,43,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }}>
            {loading ? T.saving : (
              <>
