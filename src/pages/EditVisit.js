...
function EditVisit() {
  ...
  const [form, setForm] = useState({
    ...
    city: '', visit_date: '', notes: '', rating: 5,
    latitude: '', longitude: '',
  });
  ...
  const fetchCityCoords = async (cityName, countryNameEn) => {
    ...
    try {
      const res = await axios.get(
        `https://nominatim.openstreetmap.org/search?city=${cityName}&country=${countryNameEn}&format=json&limit=1`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (photo) {
        ...
        await axios.post(`http://127.0.0.1:8000/api/visits/${id}`, formData, {
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await axios.put(`http://127.0.0.1:8000/api/visits/${id}`, form, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '28px 20px' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ position: 'relative', marginBottom: '12px' }}>
              ...
              <input
                ...
                style={{ ...inputStyle, padding: '9px 36px 9px 14px' }}
              />
            </div>
            <div style={{
              display: 'flex', flexWrap: 'wrap', gap: '8px',
              maxHeight: '180px', overflowY: 'auto', paddingLeft: '4px'
            }}>
              {filteredCountries.length === 0 ? (
                ...
                <div key={c.code}
                  onClick={() => {
                    ...
                    setCitySearch('');
                    setShowCities(false);
                    fetchCities(c.nameEn);
                  }}
                  style={{
                    padding: '6px 12px', borderRadius: '20px', cursor: 'pointer',
                    background: form.country_code === c.code
                      ? 'linear-gradient(135deg, #C0392B, #E74C3C)'
                      : 'rgba(255,255,255,0.06)',
                    color: form.country_code === c.code ? 'white' : 'rgba(255,255,255,0.5)',
                    border: `1px solid ${form.country_code === c.code ? 'transparent' : 'rgba(255,255,255,0.08)'}`,
                    fontSize: '12px', fontWeight: '600',
                    display: 'flex', alignItems: 'center', gap: '6px',
                    transition: 'all 0.2s'
                  }}
                >
                  <img src={getFlagUrl(c.code)} alt="" style={{ width: '16px', borderRadius: '2px' }} />
                  {c.name}
                </div>
              ))}
            </div>
            {form.country && (
              <div style={{
                ...
                <img src={getFlagUrl(form.country_code)} alt="" style={{ width: '18px', borderRadius: '3px' }} />
                ✅ تم اختيار: <strong style={{ color: 'white' }}>{form.country}</strong>
              </div>
            )}
          </div>
          ...
          <div style={{ ...cardStyle, position: 'relative' }}>
            <label style={labelStyle}>🏙️ اختر المدينة</label>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                placeholder={
                  ...
                  !form.country ? 'اختر دولة أولاً' :
                  cities.length > 0 ? `اضغط لاختيار مدينة (${cities.length} متاحة)` :
                  'لا توجد مدن'
                }
                ...
                style={{
                  ...
                  cursor: (!form.country || cities.length === 0) ? 'not-allowed' : 'text',
                  opacity: (!form.country || loadingCities) ? 0.4 : 1
                }}
              />
              ...
              {showCities && cities.length > 0 && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {citySearch
                      ? `${filteredCities.length} نتيجة من ${cities.length}`
                  ...
                  {filteredCities.length === 0 ? (
                    ...
                    <div key={i}
                      onMouseDown={() => {
                        setCitySearch(city);
                        setShowCities(false);
                        fetchCityCoords(city, selectedCountry?.nameEn || form.country);
                      }}
                      style={{
                        padding: '11px 16px',
                        fontSize: '13px',
                        cursor: 'pointer',
                        color: form.city === city ? 'white' : 'rgba(255,255,255,0.65)',
                        background: form.city === city ? 'rgba(192,57,43,0.2)' : 'transparent',
                        borderBottom: i < filteredCities.length - 1
                    ...
                    >
                      {city}
                      {form.city === city && (
                        <span style={{ color: '#E74C3C', fontSize: '13px' }}>✓</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {loadingCoords && (
              <div style={{
                ...
                color: 'rgba(255,255,255,0.3)', textAlign: 'center'
              }}>📍 جاري تحديد إحداثيات المدينة...</div>
            )}
            ...
            {form.city && !loadingCoords && (
              <div style={{
                ...
                {form.latitude && (
                  <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '11px', marginRight: 'auto' }}>
                    ✓ تم تحديد الموقع
                  </span>
                )}
              </div>
              ...
              {[1, 2, 3, 4, 5].map((s) => (
                <span key={s} onClick={() => setForm({ ...form, rating: s })}
                  style={{
                    ...
                    filter: s <= form.rating ? 'none' : 'grayscale(1) opacity(0.2)',
          ...
          {/* الصورة */}
          <div style={cardStyle}>
            <label style={labelStyle}>📸 صورة الرحلة</label>
            {(photoPreview || currentPhoto) && (
              <div style={{ marginBottom: '12px', position: 'relative' }}>
                <img
                  src={photoPreview || currentPhoto} alt="preview"
            ...
            <label style={{
              ...
              {photoPreview ? 'تغيير الصورة' : currentPhoto ? 'استبدال الصورة' : 'رفع صورة جديدة'}
          ...
          <button type="submit" disabled={saving} style={{
            ...
            background: saving ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #C0392B, #E74C3C)',
            color: saving ? 'rgba(255,255,255,0.3)' : 'white',
            border: 'none', borderRadius: '16px',
            fontSize: '15px', fontWeight: '800',
            cursor: saving ? 'not-allowed' : 'pointer',
            fontFamily: 'Tajawal, sans-serif',
            boxShadow: saving ? 'none' : '0 8px 24px rgba(192,57,43,0.4)'
          }}>
